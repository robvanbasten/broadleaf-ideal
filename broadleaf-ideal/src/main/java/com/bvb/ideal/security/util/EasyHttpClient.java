...
public class EasyHttpClient extends DefaultHttpClient {
	...
	public String get(String url, String request) {
		HttpPost getReq = new HttpPost(url);
		try {
			getReq.setEntity(new StringEntity(request, "UTF-8"));
		} catch (UnsupportedEncodingException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
//		HttpPost postMethod getMethod.setEntity(new StringEntity(request, "UTF-8"));
		InputStream content = null;
		try {
			content = execute(getReq).getEntity().getContent();
			ByteArrayOutputStream bout = new ByteArrayOutputStream();
	 ...
	 * @return the response string, null if there was an error
	 */
	public InputStream getStream(String url, String request) {
		HttpPost getReq = new HttpPost(url);
		try {
			getReq.setEntity(new StringEntity(request, "UTF-8"));
		} catch (UnsupportedEncodingException e1) {
